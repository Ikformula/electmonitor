...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            Current polling station assignment for this agent:
            <strong>
                @if(!is_null($user->getPollingStation()))
                    {{$user->getPollingStation()->unit_number}}
                @else
                    None
                @endif
            </strong>
    ...
    @if($user->hasRole('field_agent'))
        ...
        <form action="{{route('admin.field_agent.assign.store', $user)}}" method="post">
            {{csrf_field()}}
            <div class="row mt-4 mb-4">
                <div class="col">
                    <div class="form-group row">
                        <label class="col-md-2 form-control-label" for="timezone">Assign Polling Station</label>
                        <div class="col-md-8">
    ...
    @else
        <strong>User needs to be made a field agent before they can be assigned to a polling
            station</strong>
    @endif
        </div>
    </div>
